...
<center>
   <video id="outbreaks" src='video/outbreak_map.mp4' width='960px' loop='true' onclick='this.paused ? this.play() : this.pause()'/>
</center>
<script>
outbreaks = document.getElementById("outbreaks");
outbreaks.current = 0;
outbreaks.times = [ 96, 137, 158, 214, 233, 256, 283, 343, 356, 1000 ]
outbreaks.addEventListener("timeupdate", function(){
    for (t = 0; t < this.times.length; t++) {
      if (this.currentTime < this.times[t]/10) {
          if (this.current < t) {
              this.pause();
          }
          this.current = t;
